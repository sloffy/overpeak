document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    ascentsListEl.innerHTML = '<p>Загрузка данных...</p>';

    const expeditions = await fetchData('/api/expeditions');

    if (expeditions.length === 0) {
      ascentsListEl.innerHTML = '<p>Восхождения не найдены</p>';
      return;
    }
    ...
    // Создаем HTML для списка восхождений
    const html = `
